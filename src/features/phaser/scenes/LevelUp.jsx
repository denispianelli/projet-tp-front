 ...
 * Cette scène affiche une modale avec des options de bonus pour le personnage
 * à chaque fois qu'il monte de niveau.
 */
export default class LevelUp extends Scene {
  ...
  showLevelUpModal() {
    ...
    const firstCardX = (this.cameras.main.width - (cardWidth * 3 + cardSpacing * 2)) / 2;
    ...
    bonuses.forEach(([name, level], index) => {
      ...
      const borderColor = this.backgroundColors[
  ...
  applySelectedBonus(selectedBonus) {
    ...
    switch (selectedBonus) {
      case 'Level Projectile':
        ...
        this.character.numberOfProjectiles = (this.character.numberOfProjectiles || 1) + 1;
        break;
