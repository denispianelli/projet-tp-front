...
export default class WaveManager {
  ...
  static init(scene) {
    ...
    this.enemyWavesDuration = 3000; // Durée de chaque vague
    ...
    if (scene.character.health > 0) {
      ...
      const spawnWave = () => {
        for (let i = 0; i < wave.enemies.length; i += 1) {
          ...
          const spawnX = scene.character.x + distanceFromCharacter * Math.cos(angle);
          const spawnY = scene.character.y + distanceFromCharacter * Math.sin(angle);
