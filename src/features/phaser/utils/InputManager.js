...
export default class InputManager {
  ...
  // Créer des touches pour les contrôles WASD, ZQSD et les flèches directionnelles
    ...
    this.keysZQSD = this.scene.input.keyboard.addKeys('Z,Q,S,D');
    this.keysArrows = this.scene.input.keyboard.createCursorKeys();
    // Écouter l'événement de pression de la touche ESC pour mettre en pause le jeu

    this.scene.input.keyboard.on('keydown-ESC', this.escEventHandler);
  }

  removeInput() {
    this.scene.input.keyboard.removeAllKeys(false, true);
    this.scene.input.keyboard.off('keydown-ESC', this.escEventHandler);
  }
