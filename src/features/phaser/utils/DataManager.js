...
export default class DataManager {
  ...
  constructor(scene) {
    this.scene = scene;
    this.baseUrl = 'http://localhost:3001/v1/api/';
  }

  /**
   * Récupère le nombre de pièces possédées par le joueur.
   * @param {string} token - Le jeton d'authentification du joueur.
   * @returns {Promise<number|null>} - Une promesse résolue avec le nombre de
   * pièces, ou null en cas d'erreur.
   */
   ...
   * @returns {Promise<object[]|null>} - Une promesse résolue avec les données
   * des personnages, ou null en cas d'erreur.
   */
   ...
   * @returns {Promise<object[]|null>} - Une promesse résolue avec les données
   * des niveaux, ou null en cas d'erreur.
   */
   ...
   * @param {string} token - Le jeton d'authentification du joueur.
   * @returns {Promise<number[]|null>} - Une promesse résolue avec les
   * identifiants des personnages débloqués, ou null en cas d'erreur.
   */
  async getUserUnlockedCharacters(token) {
    try {
      const response = await fetch(`${this.baseUrl}user/characters`, {
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
        },
   ...
   * @returns {Promise<object|null>} - Une promesse résolue avec les données
   * mises à jour du joueur, ou null en cas d'erreur.
   */
   ...
   * mises à jour du joueur, ou null en cas d'erreur.
   */
  async unlockedCharacter(characterId) {
    this.userToken = localStorage.getItem('token');

    try {
      const response = await fetch(`${this.baseUrl}user/characters/${characterId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${this.userToken}`,
